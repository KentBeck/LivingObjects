...
func TestNewString(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			str := pile.NewString(tt.value)
			if str.Value != tt.value {
				t.Errorf("NewString(%q).Value = %q, want %q", tt.value, str.Value, tt.value)
			}
			if str.Type() != pile.OBJ_STRING {
				t.Errorf("NewString(%q).Type() = %d, want %d", tt.value, str.Type(), pile.OBJ_STRING)
			}
		})
	}
}
...
func TestStringToObjectAndBack(t *testing.T) {
	str := pile.NewString("hello")
	obj := pile.StringToObject(str)

	if obj.Type() != pile.OBJ_STRING {
		t.Errorf("StringToObject(str).Type() = %d, want %d", obj.Type(), pile.OBJ_STRING)
	}

	backToString := pile.ObjectToString(obj)
...
func TestStringSubstring(t *testing.T) {
	str := pile.NewString("hello world")
...
func TestStringConcat(t *testing.T) {
	str1 := pile.NewString("hello")
	str2 := pile.NewString(" world")
...
func TestStringEqual(t *testing.T) {
	str1 := pile.NewString("hello")
	str2 := pile.NewString("hello")
	str3 := pile.NewString("world")
...
func TestGetStringValue(t *testing.T) {
	...
	strObj := pile.StringToObject(pile.NewString("hello"))
	if pile.GetStringValue(strObj) != "hello" {
		t.Errorf("GetStringValue(strObj) = %q, want %q", pile.GetStringValue(strObj), "hello")
	}
	...
	t.Run("panic with immediate value", func(t *testing.T) {
		...
		intObj := pile.MakeIntegerImmediate(int64(42))
		pile.GetStringValue(intObj) // This should panic
	})
	...
	t.Run("panic with non-string object", func(t *testing.T) {
		...
		nonStringObj := &pile.Object{
			TypeField: pile.OBJ_INSTANCE, // Not a string
		}

		pile.GetStringValue(nonStringObj) // This should panic
	})
}