...
int main() {
  ...
  TaggedValue arrayResult =
      interpreter2.executeCompiledMethod(*rawCompiledMethod);

  Object *arrayObj = arrayResult.asObject();
  std::cout << "Array object header size: " << arrayObj->header.size
            << std::endl;

  Class *arrayClass = arrayObj->getClass();
  if (arrayClass) {
    std::cout << "Array class instance size: " << arrayClass->getInstanceSize()
              << std::endl;
