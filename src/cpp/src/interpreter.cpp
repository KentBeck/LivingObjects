...
namespace smalltalk
{
      ...
      : memoryManager(memory), image(image)
  {
    ...
    // Cold-boot prepare: build dicts and install pending methods (no sends)
    prepareImage(memoryManager);
  ...
  void Interpreter::sendMessageBytecode()
  {
    ...
    // Do not build/ensure any Smalltalk structures during send
  }
                                       ...
                                       const std::vector<TaggedValue> &args)
  {
    ...
    // Lookup method via class (built during bootstrap)
