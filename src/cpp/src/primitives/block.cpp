...
TaggedValue value(TaggedValue receiver, const std::vector<TaggedValue> &args,
  ...
  if (receiverObj->header.getType() != ObjectType::CONTEXT ||
      receiverObj->header.getContextType() !=
          static_cast<uint8_t>(ContextType::BLOCK_CONTEXT)) {
  ...
  MethodContext *homeContext =
      static_cast<MethodContext *>(blockContext->home.asObject());

  // The block's compiled method is stored directly in the block context's first
  // slot
  char *contextEnd =
  ...
  TaggedValue senderValue =
      TaggedValue::fromObject(interpreter.getCurrentContext());
  ...
  TaggedValue result =
      interpreter.executeMethodContext(blockMethodContext, blockMethod);
