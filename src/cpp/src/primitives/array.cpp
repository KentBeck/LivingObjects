 ...
 */
TaggedValue primitive_new_size(TaggedValue receiver,
  ...
  Object *instance = interpreter.getMemoryManager().allocateIndexableInstance(
      clazz, static_cast<size_t>(size));

  // Array elements are automatically initialized to nullptr by
  // allocateIndexableInstance which will be interpreted as nil when accessed
 ...
 */
TaggedValue primitive_at(TaggedValue receiver,
                         const std::vector<TaggedValue> &args,
                         Interpreter &interpreter) {
  ...
  // Get the element pointer - arrays store TaggedValue elements after instance
  // variables Need to skip instance variable slots first
 ...
 */
TaggedValue primitive_at_put(TaggedValue receiver,
                             const std::vector<TaggedValue> &args,
                             Interpreter &interpreter) {
  (void)interpreter; // Suppress unused parameter warning
  ...
  // Set the element directly - arrays store TaggedValue elements after instance
  // variables Need to skip instance variable slots first
  Class *arrayClass = obj->getClass();
  size_t instanceVarCount = arrayClass ? arrayClass->getInstanceSize() : 0;
 ...
 */
TaggedValue primitive_size(TaggedValue receiver,
                           const std::vector<TaggedValue> &args,
                           Interpreter &interpreter) {
  (void)interpreter; // Suppress unused parameter warning
  ...
  // Get array size from header - this is the indexed size, not including
  // instance variables
  size_t arraySize = obj->header.size;
...
void registerArrayPrimitives() {
  ...
  registry.registerPrimitive(PrimitiveNumbers::BASIC_NEW_SIZE,
                             ArrayPrimitives::primitive_new_size);
  registry.registerPrimitive(PrimitiveNumbers::ARRAY_AT,
                             ArrayPrimitives::primitive_at);
  registry.registerPrimitive(PrimitiveNumbers::ARRAY_AT_PUT,
                             ArrayPrimitives::primitive_at_put);
  registry.registerPrimitive(PrimitiveNumbers::ARRAY_SIZE,
                             ArrayPrimitives::primitive_size);
}
