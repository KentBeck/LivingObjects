...
namespace smalltalk
{
  ...
  namespace
  {
    inline TaggedValue *namedSlots(Object *obj)
    {
      return reinterpret_cast<TaggedValue *>(reinterpret_cast<char *>(obj) +
                                             sizeof(Object));
    }

    Object *ensureArray(Object *dict, size_t slotIndex, Interpreter &interpreter)
    {
      TaggedValue *slots = namedSlots(dict);
      Object *arr = slots[slotIndex].isPointer() ? slots[slotIndex].asObject() : nullptr;
      if (arr == nullptr)
      {
        ...
        slots[slotIndex] = TaggedValue(arr);
      }
      return arr;
    }
