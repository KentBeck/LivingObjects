...
TaggedValue at(TaggedValue receiver, const std::vector<TaggedValue> &args,
  ...
  if (index < 1 || index > static_cast<int32_t>(content.length())) {
    ...
    auto exception =
        std::make_unique<IndexError>("Index " + std::to_string(index) +
                                     " out of bounds for string of size " +
...
void checkStringReceiver(TaggedValue receiver) {
  ...
  // We'll trust that the method dispatch system only calls string primitives on
  // strings
  ...
  try {
    ...
    throw std::runtime_error(
        "Receiver must be a string (exception during object access)");
  }
}
...
void checkStringArgument(TaggedValue arg, size_t index) {
  ...
  if (!arg.isPointer()) {
    throw std::runtime_error("Argument " + std::to_string(index) +
                             " must be a string (got non-pointer)");
  }
  ...
  try {
    ...
    if (!obj) {
      throw std::runtime_error("Argument " + std::to_string(index) +
                               " must be a string (got null object)");
    }
  } catch (...) {
    throw std::runtime_error(
        "Argument " + std::to_string(index) +
        " must be a string (exception during object access)");
  }
}

TaggedValue concatenate(TaggedValue receiver,
 ...
 */
TaggedValue asSymbol(TaggedValue receiver, const std::vector<TaggedValue> &args,
                     Interpreter &interpreter) {
  (void)interpreter;
  checkArgumentCount(args, 0);
...
void registerStringPrimitives() {
  ...
  registry.registerPrimitive(PrimitiveNumbers::STRING_CONCAT,
                             StringPrimitives::concatenate);
  registry.registerPrimitive(PrimitiveNumbers::STRING_SIZE,
                             StringPrimitives::size);
  registry.registerPrimitive(PrimitiveNumbers::STRING_AS_SYMBOL,
                             StringPrimitives::asSymbol);
}
