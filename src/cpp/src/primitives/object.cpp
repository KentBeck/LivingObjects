 ...
 */
TaggedValue primitive_new(TaggedValue receiver, const std::vector<TaggedValue>& args, Interpreter& interpreter) {
    ...
    if (clazz->isIndexable()) {
        throw PrimitiveFailure("Cannot create indexable object without size - use basicNew:");
    }
 ...
 */
TaggedValue primitive_basic_new(TaggedValue receiver, const std::vector<TaggedValue>& args, Interpreter& interpreter) {
    // Check argument count
 ...
 */
TaggedValue primitive_basic_new_size(TaggedValue receiver, const std::vector<TaggedValue>& args, Interpreter& interpreter) {
    // Check argument count
    ...
    if (clazz->isByteIndexable()) {
        ...
        instance = interpreter.getMemoryManager().allocateIndexableInstance(clazz, static_cast<size_t>(size));
    }
...
void registerObjectPrimitives() {
    ...
    registry.registerPrimitive(PrimitiveNumbers::NEW, ObjectPrimitives::primitive_new);
    registry.registerPrimitive(PrimitiveNumbers::BASIC_NEW, ObjectPrimitives::primitive_basic_new);
    registry.registerPrimitive(PrimitiveNumbers::BASIC_NEW_SIZE, ObjectPrimitives::primitive_basic_new_size);
    registry.registerPrimitive(PrimitiveNumbers::IDENTITY_HASH, ObjectPrimitives::primitive_identity_hash);
    registry.registerPrimitive(PrimitiveNumbers::CLASS, ObjectPrimitives::primitive_class);
}
