...
SimpleParser::SimpleParser(std::string input) : input_(std::move(input)) {}
...
ASTNodePtr SimpleParser::parseStatements() {
  ...
  if (statements->getStatements().size() == 1) {
    return std::move(
        const_cast<std::vector<ASTNodePtr> &>(statements->getStatements())[0]);
  }
...
ASTNodePtr SimpleParser::parseStatement() {
  ...
  // For statements, just parse as expression (assignment is now part of
  // expression parsing)
...
ASTNodePtr SimpleParser::parseAssignmentExpression() {
  ...
  if (isAlpha(peek())) {
    ...
    if (!isAtEnd() && peek() == ':' && pos_ + 1 < input_.size() &&
      ...
      return std::make_unique<AssignmentNode>(std::move(identifier),
                                              std::move(value));
    }
  }
...
ASTNodePtr SimpleParser::parseBinaryMessage() {
  ...
  while (!isAtEnd()) {
    ...
    if (isBinarySelector()) {
      ...
      left = std::make_unique<MessageSendNode>(std::move(left), selector,
                                               std::move(args));
    } else {
      break;
    }
...
ASTNodePtr SimpleParser::parseFactor() { return parseUnary(); }
...
ASTNodePtr SimpleParser::parseUnary() {
  ...
  while (!isAtEnd()) {
    ...
    if (isAlpha(peek())) {
      ...
      // Check if this is followed by something that would make it NOT a unary
      // message
      skipWhitespace();
      ...
      // If followed by operators or end of input, it's a unary message
        ...
        receiver = std::make_unique<MessageSendNode>(std::move(receiver),
...
ASTNodePtr SimpleParser::parseInteger() {
  ...
  try {
    ...
    return std::make_unique<LiteralNode>(
        TaggedValue(static_cast<int32_t>(value)));
...
ASTNodePtr SimpleParser::parseArrayLiteral() {
  ...
  while (!isAtEnd() && peek() != ')') {
    ...
    if (isDigit(peek()) || (peek() == '-' && pos_ + 1 < input_.size() &&
                            isDigit(input_[pos_ + 1]))) {
      ...
      if (name == "true") {
        ...
        element =
            std::make_unique<LiteralNode>(TaggedValue::fromBoolean(false));
      } else if (name == "nil") {
        ...
        Symbol *symbol = Symbol::intern(name);
        element =
            std::make_unique<LiteralNode>(TaggedValue::fromObject(symbol));
      }
    } else {
...
bool SimpleParser::isAtEnd() const { return pos_ >= input_.size(); }

bool SimpleParser::isDigit(char c) const { return c >= '0' && c <= '9'; }
...
ASTNodePtr SimpleParser::parseBlock() {
  ...
  auto blockNode =
      std::make_unique<BlockNode>(std::move(parameters), std::move(body));
...
void SimpleParser::error(const std::string &message) {
  throw std::runtime_error("Parse error at position " + std::to_string(pos_) +
                           ": " + message);
}
  ...
  while (!isAtEnd()) {
    ...
    if (isAlpha(peek())) {
      ...
      if (!isAtEnd() && peek() == ':') {
        ...
        receiver = std::make_unique<MessageSendNode>(
            std::move(receiver), fullSelector, std::move(args));
      } else {
