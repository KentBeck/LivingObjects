...
SmalltalkImage::SmalltalkImage()
    : imageVersion_("1.0.0"), creationTime_(ImageUtils::getCurrentTimestamp()),
...
bool SmalltalkImage::loadSourceFile(const std::string &filename) {
  try {
    ...
    std::cerr << "Error loading source file " << filename << ": " << e.what()
              << std::endl;
    return false;
  }
}

bool SmalltalkImage::loadSourceFromString(const std::string &content,
...
bool SmalltalkImage::loadSourceDirectory(const std::string &directory) {
  try {
    ...
    std::vector<std::string> sourceFiles =
        ImageUtils::findSourceFiles(directory);

    bool allSuccess = true;
    for (const auto &filename : sourceFiles) {
      if (!loadSourceFile(filename)) {
        std::cerr << "Warning: Failed to load source file: " << filename
                  << std::endl;
        allSuccess = false;
      }
    }
    ...
    std::cerr << "Error loading source directory " << directory << ": "
              << e.what() << std::endl;
    return false;
  }
}

bool SmalltalkImage::loadSourceFiles(
...
bool SmalltalkImage::saveImage(const std::string &filename) const {
  try {
    ...
    if (!file.is_open()) {
      std::cerr << "Error: Could not create image file: " << filename
                << std::endl;
      return false;
    }
...
bool SmalltalkImage::loadImage(const std::string &filename) {
  try {
    ...
    std::cout << "  Created: "
              << ImageUtils::formatTimestamp(header.creationTime) << std::endl;
...
void SmalltalkImage::initializeFreshImage() {
  ...
  setGlobal("Object",
            TaggedValue(static_cast<Object *>(ClassUtils::getObjectClass())));
  setGlobal("Class",
            TaggedValue(static_cast<Object *>(ClassUtils::getClassClass())));
  setGlobal("Integer",
            TaggedValue(static_cast<Object *>(ClassUtils::getIntegerClass())));
  setGlobal("String",
            TaggedValue(static_cast<Object *>(ClassUtils::getStringClass())));
  setGlobal("Symbol",
            TaggedValue(static_cast<Object *>(ClassUtils::getSymbolClass())));
  setGlobal("Boolean",
            TaggedValue(static_cast<Object *>(ClassUtils::getBooleanClass())));
...
size_t SmalltalkImage::getGlobalCount() const { return globals_.size(); }
...
// evaluate(code) method moved to smalltalk_image_interpreter.cpp to avoid
// circular dependency

// evaluate(code, interpreter) method moved to smalltalk_image_interpreter.cpp
...
SmalltalkImage::getMetadata() const {
  return metadata_;
}

void SmalltalkImage::setMetadata(const std::string &key,
...
bool SmalltalkImage::readHeader(std::ifstream &file, ImageHeader &header) {
  ...
  if (header.version != IMAGE_VERSION) {
    std::cerr << "Error: Unsupported image file version: " << header.version
              << std::endl;
    return false;
  }
...
bool SmalltalkImage::writeClasses(std::ofstream &file) const {
  ...
  for (Class *clazz : classes) {
    ...
    file.write(reinterpret_cast<const char *>(&methodCount),
               sizeof(methodCount));
  }
...
bool SmalltalkImage::writeTaggedValue(std::ofstream &file,
                                      const TaggedValue &value) const {
...
bool SmalltalkImage::readTaggedValue(std::ifstream &file, TaggedValue &value) {
  ...
  if (file.good()) {
    // This is a simplified reconstruction - in reality we'd need proper object
    // reconstruction
...
bool SmalltalkImage::writeString(std::ofstream &file,
...
bool SmalltalkImage::parseSourceCode(const std::string &source,
                                     const std::string &filename) {
  // For now, just evaluate the source as expressions
  // In a full implementation, this would parse class definitions, method
  // definitions, etc.
  ...
  try {
    ...
    std::cerr << "Error parsing source " << filename << ": " << e.what()
              << std::endl;
    return false;
  }
}
...
bool SmalltalkImage::parseMethodDefinition(const std::string &source,
                                           Class *targetClass) {
