...
std::unique_ptr<CompiledMethod>
SimpleCompiler::compile(const MethodNode &method) {
  ...
  // Add implicit return of last expression (if no explicit return was
  // generated)
  if (compiledMethod->getBytecodes().empty() ||
      compiledMethod->getBytecodes().back() !=
          static_cast<uint8_t>(Bytecode::RETURN_STACK_TOP)) {
    compiledMethod->addBytecode(
        static_cast<uint8_t>(Bytecode::RETURN_STACK_TOP));
  }
...
void SimpleCompiler::compileNode(const ASTNode &node, CompiledMethod &method) {
  ...
  else if (const auto *messageSend =
               dynamic_cast<const MessageSendNode *>(&node)) {
  ...
  } else if (const auto *arrayLiteral =
...
void SimpleCompiler::compileReturn(const ReturnNode &node,
                                   CompiledMethod &method) {
  compileNode(*node.getValue(), method);
  method.addBytecode(static_cast<uint8_t>(Bytecode::RETURN_STACK_TOP));
}

void SimpleCompiler::compileLiteral(const LiteralNode &node,
                                    CompiledMethod &method) {
...
void SimpleCompiler::compileMessageSend(const MessageSendNode &node,
                                        CompiledMethod &method) {
  // Compile receiver
...
void SimpleCompiler::compileBlock(const BlockNode &node,
                                  CompiledMethod &method) {
  // Create a separate compiled method for the block on the heap
  auto blockMethod = std::make_unique<CompiledMethod>();

  // Create a separate compiler instance for the block with its own temp var
  // context
  ...
  // IMPORTANT: We also need to add the block method to the image so it can be
  // found during execution For now, we'll store the pointer directly as a
  // literal
  // TODO: Add proper block method registration to image
  int blockMethodIndex = method.addLiteral(
      TaggedValue::fromObject(reinterpret_cast<Object *>(blockMethodPtr)));
  ...
  method.addOperand(
      static_cast<uint32_t>(node.getParameters().size())); // parameter count
}

void SimpleCompiler::compileSequence(const SequenceNode &node,
                                     CompiledMethod &method) {
  ...
  for (size_t i = 0; i < statements.size(); i++) {
    ...
    // The last statement's result should remain on the stack as the sequence
    // result
...
void SimpleCompiler::compileVariable(const VariableNode &node,
                                     CompiledMethod &method) {
...
void SimpleCompiler::compileAssignment(const AssignmentNode &node,
                                       CompiledMethod &method) {
...
void SimpleCompiler::compileArrayLiteral(const ArrayLiteralNode &node,
                                         CompiledMethod &method) {
  // Simple approach: just store the array elements and generate runtime array
  // creation
  ...
  // Push size
  method.addBytecode(static_cast<uint8_t>(Bytecode::PUSH_LITERAL));
  method.addOperand(method.addLiteral(
      TaggedValue::fromSmallInteger(static_cast<int32_t>(elements.size()))));
  ...
  for (size_t i = 0; i < elements.size(); i++) {
    ...
    method.addBytecode(static_cast<uint8_t>(Bytecode::PUSH_LITERAL));
    method.addOperand(method.addLiteral(
        TaggedValue::fromSmallInteger(static_cast<int32_t>(i + 1))));
    ...
    uint32_t atPutIndex =
        method.addLiteral(TaggedValue::fromObject(atPutSymbol));
    method.addBytecode(static_cast<uint8_t>(Bytecode::SEND_MESSAGE));
