...
SmalltalkException::SmalltalkException(const std::string& message, const std::string& exceptionClass)
    : Object(ObjectType::OBJECT, 0), message_(message), exceptionClass_(exceptionClass) {
...
std::unique_ptr<SmalltalkException> ExceptionHandler::fromStdException(const std::exception& e) {
    std::string message = e.what();
    
    // Map common C++ exceptions to Smalltalk exceptions
    if (message.find("Division by zero") != std::string::npos || message.find("ZeroDivisionError") != std::string::npos) {
        return std::make_unique<ZeroDivisionError>(message);
    } else if (message.find("Undefined variable") != std::string::npos || message.find("NameError") != std::string::npos) {
    ...
    } else if (message.find("Index") != std::string::npos || message.find("bounds") != std::string::npos) {
...
TaggedValue ExceptionHandler::handleException(SmalltalkException& exception, Interpreter& interpreter) {
    // Capture stack trace
...
void ExceptionHandler::throwException(std::unique_ptr<SmalltalkException> exception) {
    // Convert to C++ exception with proper type information
    std::string errorMessage = exception->getExceptionClass();
    throw std::runtime_error(errorMessage);
}

bool ExceptionHandler::shouldCatch(const SmalltalkException& exception, const std::string& handlerClass) {
