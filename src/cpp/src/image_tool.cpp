...
void printUsage(const char *programName) {
  ...
  std::cout << "Commands:\n";
  std::cout
      << "  create <image_file>                    Create a fresh image\n";
  std::cout << "  load <source_dir> <image_file>         Load source files and "
               "save image\n";
  std::cout << "  loadfiles <file1> <file2> ... <image> Load specific files "
               "and save image\n";
  std::cout
      << "  info <image_file>                      Show image information\n";
  std::cout << "  run <image_file> <expression>          Load image and "
               "evaluate expression\n";
  std::cout << "  eval <expression>                      Evaluate expression "
               "in fresh image\n\n";
  std::cout << "Examples:\n";
  std::cout << "  " << programName << " create my_image.st\n";
  std::cout << "  " << programName << " load src/ my_image.st\n";
  std::cout << "  " << programName
            << " loadfiles Point.st Rectangle.st my_image.st\n";
...
int loadSourceDirectory(const std::string &sourceDir,
  ...
  if (manager.saveImageToFile(imageFile)) {
    auto *image = manager.getCurrentImage();
    std::cout << "Image created successfully!" << std::endl;
    std::cout << "  Source files: " << image->getSourceFiles().size()
              << std::endl;
    std::cout << "  Classes: " << image->getClassCount() << std::endl;
...
int loadSourceFiles(const std::vector<std::string> &sourceFiles,
                    const std::string &imageFile) {
  std::cout << "Loading " << sourceFiles.size() << " source files..."
            << std::endl;
...
int showImageInfo(const std::string &imageFile) {
  ...
  if (ImageUtils::getImageInfo(imageFile, version, creationTime, classCount)) {
    ...
    std::cout << "  Created: " << ImageUtils::formatTimestamp(creationTime)
              << std::endl;
    ...
    if (manager.loadImageFromFile(imageFile)) {
      ...
      std::cout << "  Globals: " << image->getGlobalCount() << std::endl;
      std::cout << "  Modified: "
                << ImageUtils::formatTimestamp(image->getModificationTime())
                << std::endl;

      // Show loaded source files
      auto sourceFiles = image->getSourceFiles();
      if (!sourceFiles.empty()) {
        std::cout << "  Source files (" << sourceFiles.size()
                  << "):" << std::endl;
        for (const auto &file : sourceFiles) {
...
int main(int argc, char *argv[]) {
  ...
  try {
    if (command == "create") {
      if (argc != 3) {
        std::cerr << "Usage: " << argv[0] << " create <image_file>"
                  << std::endl;
        return 1;
      }
      ...
      if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " load <source_dir> <image_file>"
                  << std::endl;
        return 1;
      }
      ...
      if (argc < 4) {
        std::cerr << "Usage: " << argv[0]
                  << " loadfiles <file1> <file2> ... <image_file>" << std::endl;
        return 1;
      }
      ...
      if (argc != 4) {
        std::cerr << "Usage: " << argv[0] << " run <image_file> <expression>"
                  << std::endl;
        return 1;
      }
