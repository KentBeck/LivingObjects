...
public:
    SmalltalkException(const std::string& message = "", const std::string& exceptionClass = "Exception");
...
public:
    MessageNotUnderstood(const std::string& receiver, const std::string& selector) 
        : SmalltalkException(receiver + " does not understand " + selector, "MessageNotUnderstood") {}
};
...
public:
    ...
    static std::unique_ptr<SmalltalkException> fromStdException(const std::exception& e);
    
    // Handle exception during bytecode execution
    static TaggedValue handleException(SmalltalkException& exception, Interpreter& interpreter);
    
    // Throw a Smalltalk exception (converts to C++ exception)
    [[noreturn]] static void throwException(std::unique_ptr<SmalltalkException> exception);
    
    // Check if an exception should be caught by a handler
    static bool shouldCatch(const SmalltalkException& exception, const std::string& handlerClass);
};
