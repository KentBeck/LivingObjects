...
enum class Bytecode : uint8_t {
  PUSH_LITERAL =
      0, // Push a literal from the literals array (followed by 4-byte index)
  PUSH_INSTANCE_VARIABLE =
      1, // Push an instance variable value (followed by 4-byte offset)
  PUSH_TEMPORARY_VARIABLE =
      2,         // Push a temporary variable value (followed by 4-byte offset)
  PUSH_SELF = 3, // Push self onto the stack
  STORE_INSTANCE_VARIABLE =
      4, // Store a value into an instance variable (followed by 4-byte offset)
  STORE_TEMPORARY_VARIABLE =
      5, // Store a value into a temporary variable (followed by 4-byte offset)
  SEND_MESSAGE = 6,     // Send a message (followed by 4-byte selector index and
                        // 4-byte arg count)
  ...
  JUMP_IF_FALSE =
      10,         // Jump if top of stack is false (followed by 4-byte target)
  POP = 11,       // Pop the top value from the stack
  DUPLICATE = 12, // Duplicate the top value on the stack
  CREATE_BLOCK = 13, // Create a block (followed by 4-byte bytecode size, 4-byte
                     // literal count, 4-byte temp var count)
...
inline size_t getInstructionSize(Bytecode bytecode) {
  ...
  case Bytecode::SEND_MESSAGE:
    return INSTRUCTION_SIZE_SEND_MESSAGE; // 1 byte opcode + 4 byte selector
                                          // index + 4 byte arg count
  case Bytecode::CREATE_BLOCK:
    return INSTRUCTION_SIZE_CREATE_BLOCK; // 1 byte opcode + 4 byte bytecode
                                          // size + 4 byte literal count + 4
                                          // byte temp var count
  ...
  default:
    return INSTRUCTION_SIZE_ONE_BYTE_OPCODE; // 1 byte opcode (includes default
                                             // for unknown bytecodes)
  }
}
