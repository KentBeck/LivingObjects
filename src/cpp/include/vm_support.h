...
public:
  // Constructors
  VMValue()
      : tagged_value_(TaggedValue::nil()), heap_object_(nullptr),
        is_immediate_(true) {}
  VMValue(TaggedValue tv)
      : tagged_value_(tv), heap_object_(nullptr), is_immediate_(true) {}
  VMValue(Object *obj)
      : tagged_value_(TaggedValue::nil()), heap_object_(obj),
        is_immediate_(false) {}
  ...
  bool is_integer() const {
    return is_immediate_ && tagged_value_.is_integer();
  }
  bool is_boolean() const {
    return is_immediate_ && tagged_value_.is_boolean();
  }
 ...
 */
SmalltalkClass *get_smalltalk_class(const VMValue &value,
 ...
 */
bool is_instance_of(const VMValue &value, SmalltalkClass *target_class,
                    const WellKnownClasses &classes);
 ...
 */
Object *box_if_needed(const VMValue &value, MemoryManager &memory,
                      const WellKnownClasses &classes);
 ...
 */
VMValue unbox_if_possible(const VMValue &value,
                          const WellKnownClasses &classes);
 ...
 */
Object *create_object(SmalltalkClass *smalltalk_class, uint32_t size,
                      MemoryManager &memory);

/**
 * Create specific object types - VM knows how to format them
 */
Object *create_array(uint32_t size, MemoryManager &memory,
                     const WellKnownClasses &classes);
Object *create_string(const std::string &content, MemoryManager &memory,
                      const WellKnownClasses &classes);
Object *create_symbol(const std::string &content, MemoryManager &memory,
                      const WellKnownClasses &classes);
Object *create_byte_array(uint32_t size, MemoryManager &memory,
                          const WellKnownClasses &classes);
 ...
 */
VMValue array_at(Object *array_obj, uint32_t index,
                 const WellKnownClasses &classes);
void array_at_put(Object *array_obj, uint32_t index, const VMValue &value,
                  const WellKnownClasses &classes);
...
void string_set_content(Object *string_obj, const std::string &content,
                        const WellKnownClasses &classes);

/**
 * ByteArray operations
 */
uint8_t byte_array_at(Object *byte_array_obj, uint32_t index,
                      const WellKnownClasses &classes);
void byte_array_at_put(Object *byte_array_obj, uint32_t index, uint8_t value,
                       const WellKnownClasses &classes);
...
ObjectInfo inspect_object(const VMValue &value,
                          const WellKnownClasses &classes);
...
public:
  ...
  VMValue primitive_add(const VMValue &left, const VMValue &right,
                        MemoryManager &memory);
  ...
  VMValue primitive_new_with_size(SmalltalkClass *class_obj, uint32_t size,
                                  MemoryManager &memory);

  // Array primitives
  VMValue primitive_array_at(const VMValue &array, const VMValue &index);
  VMValue primitive_array_at_put(const VMValue &array, const VMValue &index,
                                 const VMValue &value);
