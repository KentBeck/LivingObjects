...
class TaggedValue {
public:
  enum class Type : uint8_t { NIL = 0, BOOLEAN, INTEGER, FLOAT };
  ...
  static TaggedValue true_value() { return boolean(true); }

  static TaggedValue false_value() { return boolean(false); }
  ...
  TaggedValue(const TaggedValue &other)
      : type_(other.type_), int_val_(other.int_val_) {}
  ...
  bool is_true() const { return type_ == Type::BOOLEAN && int_val_ != 0; }

  bool is_false() const { return type_ == Type::BOOLEAN && int_val_ == 0; }
  ...
  bool operator!=(const TaggedValue &other) const { return !(*this == other); }
...
private:
  ...
  TaggedValue(Type type, uint32_t int_value)
      : type_(type), int_val_(int_value) {}
