...
/**
 * Exception thrown when a primitive fails and should fall back to Smalltalk
 * code
 */
class PrimitiveFailure : public std::exception {
public:
  explicit PrimitiveFailure(std::string message)
...
// SystemLoader / bootstrap
const int SYSTEM_LOADER_START = 5000; // SystemLoader>>start:

// Compiler / bootstrap bridge
const int COMPILER_COMPILE_IN = 5100; // Compiler>>compile:in:
} // namespace PrimitiveNumbers
