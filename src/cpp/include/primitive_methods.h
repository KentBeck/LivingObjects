...
/**
 * PrimitiveMethod represents a method implemented in C++ rather than Smalltalk
 * bytecode. These are used for basic operations like arithmetic, comparison,
 * etc.
 */
...
public:
  ...
  using PrimitiveFunction = std::function<TaggedValue(
      TaggedValue receiver, const std::vector<TaggedValue> &args,
      class Interpreter &interpreter)>;

  PrimitiveMethod(int primitiveNumber, PrimitiveFunction function);

  // Execute the primitive
  TaggedValue execute(TaggedValue receiver,
                      const std::vector<TaggedValue> &args,
                      class Interpreter &interpreter) const;
...
public:
  ...
  void registerPrimitive(int primitiveNumber,
                         PrimitiveMethod::PrimitiveFunction function);

  // Create a primitive method
  std::shared_ptr<PrimitiveMethod>
  createPrimitiveMethod(int primitiveNumber) const;
