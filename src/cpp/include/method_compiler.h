...
public:
   ...
   *
   * @param methodSource The complete method source (e.g., "ensure: aBlock |
   * result | ...")
   * @return A compiled method ready to be added to a class
   */
  static std::shared_ptr<CompiledMethod>
  compileMethod(const std::string &methodSource);
...
private:
   ...
   * Returns the selector string and modifies the source to remove the
   * signature.
   */
