...
struct SourceFile {
  ...
  SourceFile(const std::string &name, const std::string &text,
...
public:
  ...
  bool loadSourceFromString(const std::string &content,
  ...
  // Evaluate Smalltalk code using provided interpreter (breaks circular
  // dependency)
...
private:
  ...
  std::unordered_map<uint32_t, std::unique_ptr<CompiledMethod>>
      compiledMethods_;
