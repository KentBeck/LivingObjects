...
public:
  ...
  void enableBytecodeTracing(bool enabled) { traceBytecode = enabled; }

  void enableStackTracing(bool enabled) { traceStack = enabled; }

  void enableMethodCalls(bool enabled) { traceMethodCalls = enabled; }

  void enableMemoryDebug(bool enabled) { traceMemory = enabled; }
  ...
  void traceBytecodeExecution(const std::string &bytecode, uint32_t ip,
    ...
    if (!traceBytecode || debugLevel > LogLevel::DEBUG_LEVEL)
      return;
  ...
  // Method call tracing
  void traceMethodEntry(const std::string &methodName,
                        const std::string &className,
                        const std::vector<TaggedValue> &args) {
    if (!traceMethodCalls || debugLevel > LogLevel::DEBUG_LEVEL)
      return;
  ...
  void traceMethodExit(const std::string &methodName,
                       const std::string &className, TaggedValue result) {
    if (!traceMethodCalls || debugLevel > LogLevel::DEBUG_LEVEL)
      return;
  ...
  // Memory debugging
  void traceAllocation(const std::string &objectType, size_t size,
                       void *address) {
    if (!traceMemory || debugLevel > LogLevel::DEBUG_LEVEL)
      return;
  ...
  // Exception debugging
  void traceException(const std::string &exceptionType,
  ...
  void tracePerformance(const std::string &operation, double durationMs) {
    if (debugLevel > LogLevel::DEBUG_LEVEL)
      return;
  ...
  std::string taggedValueToString(TaggedValue value) {
    ...
    if (value.isInteger()) {
      ...
      ss << "<Object@" << std::hex
         << reinterpret_cast<uintptr_t>(value.asObject()) << ">";
    } else {
...
private:
  VMDebugger()
      : debugLevel(LogLevel::INFO), traceBytecode(false), traceStack(false),
        traceMethodCalls(false), traceMemory(false) {}
