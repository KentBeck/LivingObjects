...
/**
 * Method node - represents a complete method with optional temporary variables
 * and an expression body
 */
class MethodNode : public ASTNode {
public:
  explicit MethodNode(ASTNodePtr body)
      : tempVars_(), body_(std::move(body)), primitiveNumber_(-1) {}
  ...
  MethodNode(std::vector<std::string> tempVars, ASTNodePtr body,
             int primitiveNumber)
      : tempVars_(std::move(tempVars)), body_(std::move(body)),
        primitiveNumber_(primitiveNumber) {}
...
public:
  MessageSendNode(ASTNodePtr receiver, std::string selector,
                  std::vector<ASTNodePtr> arguments)
      : receiver_(std::move(receiver)), selector_(std::move(selector)),
