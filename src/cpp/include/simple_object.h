 ...
 * This represents a Smalltalk class object that defines the behavior of instances
 */
class SmalltalkClass {
public:
 ...
 * 2. Immediates (integers, booleans, nil) are TaggedValues only, never heap objects  
...
struct Object {
    ...
    Object(SmalltalkClass* st_class, uint32_t size, uint16_t identity_hash = 0)
        ...
        // Set HAS_POINTERS flag based on class (implementation will check class format)
        // For now, assume most objects have pointers
        header.set_flag(ObjectFlag::HAS_POINTERS);
    }
    
    // === ACCESSORS FOR OBJECT METADATA ===
    
    SmalltalkClass* get_class() const { 
        return smalltalk_class; 
    }
    ...
    uint16_t identity_hash() const { 
        return header.hash; 
    }
    
    void set_identity_hash(uint16_t hash) { 
        header.hash = hash; 
    }
    ...
    bool is_marked() const { 
        return header.has_flag(ObjectFlag::MARKED); 
    }
    
    void mark() { 
        header.set_flag(ObjectFlag::MARKED); 
    }
    
    void unmark() { 
        header.clear_flag(ObjectFlag::MARKED); 
    }
    ...
    bool is_forwarded() const { 
        return header.has_flag(ObjectFlag::FORWARDED); 
    }
 ...
 */
inline size_t object_size_bytes(uint32_t data_size, bool is_byte_object = false) {
