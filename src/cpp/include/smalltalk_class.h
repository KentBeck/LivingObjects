...
public:
  ...
  const MethodDictionary &getMethodDictionary() const {
    return methodDictionary_;
  }
  ...
  const std::vector<std::string> &getInstanceVariables() const {
    return instanceVariables_;
  }
  ...
  const std::vector<std::string> &getClassVariables() const {
...
public:
  Metaclass(const std::string &name, Class *instanceClass,
