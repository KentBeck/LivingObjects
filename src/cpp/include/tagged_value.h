...
public:
  ...
  static constexpr uintptr_t SPECIAL_TAG =
      0x01;                                    // 01 = Special (nil/true/false)
  ...
  explicit TaggedValue(Symbol *symbol) {
    ...
    if ((reinterpret_cast<uintptr_t>(symbol) & TAG_MASK) != 0) {
      throw std::runtime_error(
          "Symbol pointer not properly aligned for tagging");
    }
  ...
  explicit TaggedValue(Object *object) {
    ...
    if ((reinterpret_cast<uintptr_t>(object) & TAG_MASK) != 0) {
      throw std::runtime_error(
          "Object pointer not properly aligned for tagging");
    }
  ...
  // Create from double (may need to allocate on heap if doesn't fit in tagged
  // value)
  explicit TaggedValue(double doubleValue) {
    // For now, just encode a subset of floats that can be stored inline
    // A full implementation would allocate a heap object for floats that don't
    // fit
    if (doubleValue == 0.0) {
      // Encode 0.0 directly
  ...
  // Factory methods
  static TaggedValue fromObject(Object *object); // Implementation in .cpp file
  static TaggedValue fromSmallInteger(int32_t value) {
    return TaggedValue(value);
  }
  static TaggedValue fromFloat(double value) { return TaggedValue(value); }
  static TaggedValue fromBoolean(bool value) {
  ...
  int32_t getSmallInteger() const { return asInteger(); }

  bool getBoolean() const { return asBoolean(); }
