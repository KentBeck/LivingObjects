...
const uint64_t OBJECT_HEADER_FLAGS_BITS =
    8; // Now includes ObjectType (3 bits) + flags (3 bits) + 2 spare
...
enum class ObjectFlag : uint8_t {
  ...
  TAGGED_VALUE_WRAPPER =
      6 // Object wraps a TaggedValue (for immediate values on stack)
};

// Object header structure (64 bits)
struct ObjectHeader {
  uint64_t size : OBJECT_HEADER_SIZE_BITS;   // Size in slots or bytes
  uint64_t flags : OBJECT_HEADER_FLAGS_BITS; // Flags (3 bits) + ObjectType (3
                                             // bits) + 2 spare
  uint64_t hash : OBJECT_HEADER_HASH_BITS;   // Identity hash

  // Constructor
  ObjectHeader(ObjectType objectType, size_t objectSize,
               uint32_t objectHash = 0)
      : size(objectSize),
        flags(static_cast<uint64_t>(objectType) << OBJECT_TYPE_SHIFT),
        hash(objectHash) {}

  // ObjectType operations
  ObjectType getType() const {
    return static_cast<ObjectType>((flags >> OBJECT_TYPE_SHIFT) &
                                   OBJECT_TYPE_MASK);
  }
  ...
  // ContextType operations (stored in top 3 bits of hash field for CONTEXT
  // objects)
  uint8_t getContextType() const {
    return static_cast<uint8_t>((hash >> CONTEXT_TYPE_SHIFT) &
                                CONTEXT_TYPE_MASK);
  }

  void setContextType(uint8_t contextType) {
    hash = (hash & CONTEXT_HASH_MASK) |
           (static_cast<uint64_t>(contextType) << CONTEXT_TYPE_SHIFT);
  }
  ...
  // Flag operations
  void setFlag(ObjectFlag flag) { flags |= (1 << static_cast<uint8_t>(flag)); }
...
struct Object {
  ...
  Object(ObjectType objectType, size_t objectSize, Class *objectClass,
         uint32_t objectHash = 0)
...
public:
  SmallInteger(int32_t value, Class *integerClass)
      : Object(ObjectType::IMMEDIATE, sizeof(SmallInteger), integerClass),
        value_(value) {}
...
public:
  Boolean(bool value, Class *booleanClass)
      : Object(ObjectType::IMMEDIATE, sizeof(Boolean), booleanClass),
        value_(value) {}
