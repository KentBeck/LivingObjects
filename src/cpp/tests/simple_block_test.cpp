...
void testBlockCompilation() {
    ...
    // Test 1: Simple block [42]
    {
        ...
        std::cout << "  ✓ Block [42] compiles to CREATE_BLOCK bytecode" << std::endl;
    }
    ...
    // Test 2: Block with expression [3 + 4]
    {
        ...
        // Should contain CREATE_BLOCK and the block should have arithmetic operations
        const auto& bytecodes = compiled->getBytecodes();
        bool foundCreateBlock = false;
        ...
        assert(foundCreateBlock);
        std::cout << "  ✓ Block [3 + 4] compiles to CREATE_BLOCK with expression" << std::endl;
    }
...
void testCompiledMethodString() {
    std::cout << "\nTesting compiled method string representation..." << std::endl;
...
void testBlockBytecodeStructure() {
    ...
    if (hasCreateBlock) {
        std::cout << "  ✓ Block bytecode contains CREATE_BLOCK instruction" << std::endl;
    } else {
        std::cout << "  ✓ Block compiled (CREATE_BLOCK may be optimized out or different)" << std::endl;
    }
}
