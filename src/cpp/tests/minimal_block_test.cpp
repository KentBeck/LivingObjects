...
void testBlockValuePrimitive() {
    ...
    try {
        ...
        std::cout << "  ✓ Block value primitive returns correct result: " << result << std::endl;
...
void testBlockWithExpression() {
    ...
    try {
        ...
        std::cout << "  ✓ Block with expression [3 + 4] returns: " << result << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "  ✗ Block expression execution failed: " << e.what() << std::endl;
        assert(false);
    }
}
