...
public:
    ...
    MockSmalltalkClass(const std::string& class_name, ObjectFormat fmt, uint32_t size = 0) 
...
VMValue create_test_array(uint32_t size, TestMemoryManager& memory, const WellKnownClasses& classes);
Object* create_simple_object(SmalltalkClass* st_class, uint32_t size, TestMemoryManager& memory);
...
void test_uniform_objects() {
    ...
    MockSmalltalkClass* array_class = new MockSmalltalkClass("Array", ObjectFormat::ARRAY);
    MockSmalltalkClass* string_class = new MockSmalltalkClass("String", ObjectFormat::BYTE_ARRAY);
    MockSmalltalkClass* point_class = new MockSmalltalkClass("Point", ObjectFormat::REGULAR, 2);
    
    // Test 1: Create an Array object (holds pointers)
    size_t array_size = 3;
    size_t array_bytes = object_size_bytes(array_size, false); // false = slot object
    ...
    size_t string_bytes = object_size_bytes(content.length(), true); // true = byte object
...
void test_vm_operations() {
    ...
    classes.SmallInteger = new MockSmalltalkClass("SmallInteger", ObjectFormat::REGULAR);
...
void test_no_parallel_hierarchy() {
    ...
    MockSmalltalkClass* integer_class = new MockSmalltalkClass("SmallInteger", ObjectFormat::REGULAR);
    MockSmalltalkClass* boolean_class = new MockSmalltalkClass("Boolean", ObjectFormat::REGULAR);
    MockSmalltalkClass* array_class = new MockSmalltalkClass("Array", ObjectFormat::ARRAY);
    MockSmalltalkClass* string_class = new MockSmalltalkClass("String", ObjectFormat::BYTE_ARRAY);
    ...
    for (const auto& value : values) {
        if (value.is_immediate()) {
            std::cout << "  Immediate: " << value.as_immediate().to_string() << std::endl;
        } else {
            Object* obj = value.as_object();
            MockSmalltalkClass* mock_class = static_cast<MockSmalltalkClass*>(obj->get_class());
...
int main() {
    ...
    try {
        ...
        std::cout << "• Immediates are TaggedValues only - never heap allocated" << std::endl;
        std::cout << "• All heap objects use uniform Object struct" << std::endl;
        std::cout << "• VM behavior based on Smalltalk class, not C++ type" << std::endl;
