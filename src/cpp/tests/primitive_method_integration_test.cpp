...
TEST(testIntegerAdditionWithPrimitive) {
    ...
    try {
        ...
        std::cout << "✓ Integer addition primitive works: 10 + 15 = " << result.asInteger() << std::endl;
...
TEST(testMultiplePrimitiveMethods) {
    std::cout << "Testing multiple methods with different primitives..." << std::endl;
    ...
    for (const auto& test : tests) {
        std::string methodSource = "<primitive: " + std::to_string(test.primitiveNumber) + "> ^nil";
        ...
        TaggedValue result = primitiveRegistry.callPrimitive(test.primitiveNumber, receiver, args, interpreter);
        
        // For comparison operations, check boolean result
        if (test.primitiveNumber >= 3 && test.primitiveNumber <= 8) {
            bool boolResult = (test.primitiveNumber == 3) ? (test.receiver < test.arg) :
                             (test.primitiveNumber == 4) ? (test.receiver > test.arg) :
                             (test.primitiveNumber == 7) ? (test.receiver == test.arg) : false;
            EXPECT_TRUE(boolResult ? result.isTrue() : result.isFalse());
            std::cout << "✓ Primitive " << test.primitiveNumber << " (" << test.operation 
                     << "): " << test.receiver << " " << test.operation << " " << test.arg 
...
TEST(testPrimitiveWithComplexFallback) {
    ...
    std::string methodSource = "<primitive: 9999> | a b | a := 100. b := 200. ^a + b";
    ...
    try {
        ...
        // Check that fallback bytecode was generated (should have more than just RETURN)
...
int main() {
    std::cout << "Running primitive method integration tests..." << std::endl << std::endl;
    
    testIntegerAdditionWithPrimitive();
    testMultiplePrimitiveMethods();
    testPrimitiveWithComplexFallback();
    
    std::cout << std::endl << "All primitive method integration tests passed!" << std::endl;
    return 0;
}